---
name: add-npu-op
description: Add a new operator to the TileOPs project (NPU backend). Caller provides the op name, the GPU TileOPs repo root (source of truth for porting), and an optional reference API; skill creates all 7 files (manifest entry, workload, kernel, op, test, benchmark, conftest wiring) and verifies on NPU. Idempotent.
---
...
3. **`ref_api`** (optional) — PyTorch reference, e.g. `torch.nn.functional.softmax`
4. **`family`** (optional) — op family directory, e.g. `reduction` (default: `reduction`)
...
| S3 | `tileops/kernels/{family}/{op_slug}.py` | NPU kernel (TileLang-based, NPUIR target) |
...
| S7 | `tileops/{kernels,ops}/{family}/__init__.py` | Package exports (updated) |
...
4. **Verify** structure (Tier 1); runtime verification (Tier 2) happens
   after the NPU kernel component fills in the factory stubs.
...
benchmark) and stubs the TileLang kernel factory functions
(interface + docstring only). The NPU TileLang kernel *implementation*
(`@tilelang.jit(target="npuir")` + `@T.prim_func` body) is authored by a
separate NPU kernel component.

LogSumExpFwdOp and LerpTensorFwdOp are reference examples (reduction and
elementwise/multi-input families respectively), **not** universal templates.
...
### TileLang kernel factory functions (applies to S3)

The factory functions (e.g. `_{op_slug}_kernel_single`, `_{op_slug}_kernel_tiled`,
`_{op_slug}_kernel`) are **interface-only stubs** in this skill — the function
signature and docstring are preserved from the GPU version, but the
implementation body is left empty. The NPU TileLang implementation
(`@tilelang.jit(target="npuir")` + `@T.prim_func` body) is authored by a
separate NPU kernel component, not by this skill.

| # | GPU original | NPU stub (this skill) |
|---|--------------|------------------------|
| K1 | `@tilelang.jit` decorator + `@T.prim_func` body (CUDA target) | **Not written** — factory body is empty (`raise NotImplementedError` or `pass`) |
| K2 | `T.Kernel(grid, threads=threads)` inside the prim_func body | **Not written** — belongs to the NPU kernel component |
| K3 | `threads` param in factory callable signature | **Removed from the signature** — reduction ops' callable takes `block_m` (and `tile_n` for tiled variants); elementwise ops' callable takes `block_size` (see K11 for `npt`) |
| K4 | Pad N to `DEFAULT_ALIGNMENT` + masked loads (inside the body) | **Not applicable** — no body to pad; the NPU component decides padding strategy |
...
GPU version with GPU-specific parts adapted. The class calls the factory
stubs; the stubs return `None` / raise until the NPU component fills them in.
...
Create `tileops/kernels/{family}/{op_slug}.py`. The file has two parts
with different treatment:

#### Part A — TileLang kernel factory functions (interface-only stubs)

The factory functions (e.g. `_{op_slug}_kernel_single`,
`_{op_slug}_kernel_tiled`, `_{op_slug}_kernel`) are **interface-only stubs**.
Preserve the function signature and docstring from the GPU version; leave
the implementation body empty. The NPU TileLang implementation
(`@tilelang.jit(target="npuir")` + `@T.prim_func` body) is authored by a
separate NPU kernel component, not by this skill.

**Port from GPU**: Copy the factory function signatures and docstrings from
`{gpu_repo_root}/tileops/kernels/<family>/*.py`.

- **Preserve**: function name, parameter list (minus GPU-specific params:
  `threads`, and for elementwise ops also `npt`/`num_per_thread` per K11),
  docstring (which documents the algorithm the NPU component will implement).
- **Apply K1-K4 and K11** from the Standard NPU Adaptations section.
- **Body**: empty — use `raise NotImplementedError(...)` or `pass` with a
  comment marker.

Use this standard comment header above the factory functions:
...
Example stub (single-input reduction style — adapt to the GPU op's actual
factory structure):

```python
@functools.lru_cache(maxsize=64)
def _{op_slug}_kernel_single(M: int, N: int, dtype: str):
    """Build a single-tile {OpName} kernel.

    Uses `@tilelang.jit(target="npuir")` + `@T.prim_func`.
    The returned callable signature is `_func(block_m)` (no `threads`).
    Operates on raw N (no alignment padding) to avoid NPUIR vsel
    shape mismatch issues.

    [Algorithm description preserved from GPU docstring — documents what
    the NPU component will implement.]
    """
    raise NotImplementedError("NPU kernel implementation pending")


@functools.lru_cache(maxsize=64)
def _{op_slug}_kernel_tiled(M: int, N: int, dtype: str, tile_n: int):
    """Build a multi-tile {OpName} kernel.

    The returned callable signature is `_func(block_m)` (tile_n is
    baked into the closure; no `threads`).

    [Algorithm description preserved from GPU docstring.]
    """
    raise NotImplementedError("NPU kernel implementation pending")


@functools.lru_cache(maxsize=64)
def _{op_slug}_kernel(M: int, N: int, dtype: str, tile_n: int = 0):
    """Build the appropriate {OpName} kernel."""
    if tile_n > 0:
        return _{op_slug}_kernel_tiled(M, N, dtype, tile_n)
    return _{op_slug}_kernel_single(M, N, dtype)
```

Example stub (flat elementwise style — adapt to the GPU op's actual
factory structure):

```python
@functools.lru_cache(maxsize=32)
def _{op_slug}_kernel(N: int, dtype: str):
    """Build a flat {OpName} kernel.
...
- The factory function set depends on the op. A reduction op may have
  ...
  **Port the GPU factory structure (names, params, docstrings), stubbing
  each body.**
- The stubbed callable signature is op-family-specific:
  - **Reduction ops**: `block_m` (single-tile) or `(block_m, tile_n)`
    (tiled) — no `threads` (K3).
  - **Elementwise ops**: `block_size` — no `threads` or `npt` (K3 + K11);
    the GPU `threads * npt` product collapses into `block_size`.
- The dispatcher function (e.g. `_{op_slug}_kernel`) selects between
  variants and **is fully implemented** (it only calls the stubs).

#### Part B — custom_op wrapper + Kernel class (fully ported)

The `@torch.library.custom_op` wrapper and the **Kernel class** are ported
in full — they call the factory stubs, which return `None` / raise until
the NPU component fills them in.
...
- The `custom_op` wrapper signature must match the kernel's `forward`
  arguments. A single-input kernel wraps `(M, N, dtype_str, block_m,
  tile_n, x)`; a multi-input kernel wraps `(M, N, dtype_str, block_m,
  tile_n, a, b, w)`. **Follow the GPU wrapper, removing `threads`.**
...
- The Kernel class `__init__` constructs and caches the factory callable
  via `_{op_slug}_kernel(...)`. Since the factory body is stubbed, this
  will raise at runtime until the NPU component implements it — this is
  expected. The class structure (config selection, cache logic) is still
  ported so it is ready once the factory is filled in.

Update `tileops/kernels/{family}/__init__.py` to export the kernel class.
...
Since the factory functions are interface-only stubs (Part A of S3),
...
correct and will be ready once the NPU kernel component fills in the
factory implementations.

#### Tier 2 — Runtime verification (after NPU kernel component fills stubs)

Once the factory function bodies are implemented by the NPU kernel
component, run:
...
- **Elementwise / multi-input family** (3-input, broadcast, custom
  parametrization, `threads`+`npt` → `block_size` per K11):
  - `tileops/kernels/elementwise/lerp_tensor.py`
  - `tileops/ops/elementwise/lerp_tensor.py`
  - `tests/ops/test_lerp_tensor.py`
  - `benchmarks/ops/bench_lerp_tensor.py`
