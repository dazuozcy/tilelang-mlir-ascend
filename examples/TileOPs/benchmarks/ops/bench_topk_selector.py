"""Benchmark for the top-k selector op (NPU).

Adapted from TileOPs ``benchmarks/ops/bench_topk_selector.py``.  Workload
shapes, dtypes, and ``topk`` come from the ops manifest; roofline FLOP and
byte counts come from the op's ``eval_roofline()`` via
:class:`ManifestBenchmark`.

Multi-input note: TopkSelectorOp has 3 tensor inputs (index_score, starts,
ends), so ``workloads_to_params`` (single-input contract) cannot be used;
the GPU ``workload_field_params`` helper is ported locally instead.
"""
...
def _workload_field_params(workloads: list, keys: tuple) -> list:
    """Turn manifest workload dicts into pytest params (multi-input pattern).

    Ported from the GPU ``benchmarks/benchmark_base.workload_field_params``:
...
_TOPK_SELECTOR_PARAMS = _workload_field_params(
...
def test_topk_selector_bench(
    ...
    test = TopkSelectorTestBaseline(batch, seq_len, seq_len_kv, kv_group, topk, in_dtype, out_dtype)
