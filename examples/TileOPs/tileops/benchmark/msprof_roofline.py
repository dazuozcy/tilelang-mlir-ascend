...
SOC_CONFIG = {
    'Ascend910B2C': {
        'num_cores': 48, 'cacheline_size': 128,
        'vec_peak_ppc': {'FP32': 64, 'MISC': 32, 'FP16': 128, 'INT32': 64, 'INT16': 128, 'S32': 64, 'S16': 128},
        'cube_peak_ppc': 4096,
    ...
    'Ascend910A': {
        'num_cores': 32, 'cacheline_size': 64,
        'vec_peak_ppc': {'FP32': 64, 'MISC': 32, 'FP16': 128, 'INT32': 64, 'INT16': 128, 'S32': 64, 'S16': 128},
        'cube_peak_ppc': 4096,
        'cube_roofline_multiplier': 384,
        'gm_bw': 1.2, 'l2_bw': 6.0,
...
VEC_INSTR_MAPPING = {
    'Vector FP32': 'FP32', 'Vector FP16': 'FP16',
    'Vector INT32': 'INT32', 'Vector INT16': 'INT16',
    'Vector S32': 'S32', 'Vector S16': 'S16',
    'Vector Misc': 'MISC',
}
        ...
        for graph in j.get('multiple_rooflines', []):
            ...
            for r in graph.get('rooflines', []):
                ...
                roofline_entries.append({
                    'title': title, 'bw_name': bw_name, 'bw': bw,
                    'AI': ai, 'performance': perf,
                    'computility': computility, 'ratio': ratio,
                    'computility_name': computility_name,
                })

    return {
        'base_info': base_info,
        'roofline_entries': roofline_entries, 'config': config,
    }
...
def compute_from_csvs(dir_path, config):
    ...
    # ---- Vector computility ----
    sum_fp32_cycles = sum(safe_float(r.get('aiv_vec_fp32_ratio')) * safe_float(r.get('aiv_total_cycles')) for r in vec_rows)
    sum_misc_cycles = sum(safe_float(r.get('aiv_vec_misc_ratio')) * safe_float(r.get('aiv_total_cycles')) for r in vec_rows)
    sum_vec_cycles = sum(safe_float(r.get('aiv_vec_ratio')) * safe_float(r.get('aiv_total_cycles')) for r in vec_rows)
    ...
    return {
        'op_type': op_type, 'duration': duration, 'freq': freq, 'block_dim': block_dim,
        'cube_fops_csv': cube_fops_csv, 'cube_fops': cube_fops, 'cube_roofline_multiplier': cube_roofline_multiplier,
        'cube_peak_ppc': cube_peak_ppc, 'cube_computility': cube_computility / 1e12,
        'sum_cube_cycles': sum_cube_cycles, 'sum_cube_ratio': sum_cube_ratio,
        'vec_fops': vec_fops, 'vec_peak_ppc_weighted': vec_peak_ppc_weighted,
        'fp32_prop': fp32_prop, 'misc_prop': misc_prop,
        'vec_computility': vec_computility / 1e12,
        'sum_vec_cycles': sum_vec_cycles,
        'total_fops': total_fops, 'combined_computility': comp_tops,
        'total_l2_read_miss': total_l2_read_miss, 'total_bytes': total_bytes,
        'cacheline_size': cacheline_size, 'num_cores': num_cores,
        'AI': ai, 'performance': perf_tops, 'computility': comp_tops,
        'bw': gm_bw, 'l2_bw': l2_bw, 'ratio': ratio,
        'l2_ratio': l2_ratio,
    }
...
def main():
    ...
    print("=" * 90)
    print("  Roofline Metrics Comparison: visualize_data.bin (Method 1)  vs  CSV Formulas (Method 2)")
    print("=" * 90)
    ...
    for e in bin_data['roofline_entries']:
        print(f"  {e['title']:<22} {e['bw_name']:<22} {e['bw']:>8.3f} {e['AI']:>14.6f} {e['performance']:>12.6f} {e['computility']:>12.6f} {e['ratio']:>10.6f}")
    ...
    if csv_m['vec_fops'] > 0:
        ...
        print(f"    fp32 proportion             : {csv_m['fp32_prop']:.9f}  ({csv_m['fp32_prop']*100:.6f}%)")
        print(f"    misc proportion             : {csv_m['misc_prop']:.9f}  ({csv_m['misc_prop']*100:.6f}%)")
    ...
    print(f"    Roofline peak               : min({csv_m['AI']:.6f}x{csv_m['bw']}, {csv_m['computility']:.6f}) = {rl_peak:.9f}")
    ...
    bin_gm_entries = [e for e in bin_data['roofline_entries']
                      if e.get('title') == 'GM/L2' and 'GM Read + Write' in e.get('bw_name', '')]
    bin_l2_entries = [e for e in bin_data['roofline_entries']
                      if e.get('title') == 'GM/L2' and 'L2 Read + Write' in e.get('bw_name', '')]

    if not bin_gm_entries:
        bin_gm_entries = [e for e in bin_data['roofline_entries']
                          if 'GM' in e.get('bw_name', '') and ('GM/L2' in e.get('title', '') or 'Memory Unit' in e.get('title', ''))]

    # CSV metrics dict (GM path)
    csv_gm = {
        'bw': csv_m['bw'], 'fops': csv_m['total_fops'],
        'total_bytes': csv_m['total_bytes'],
        'AI': csv_m['AI'], 'performance': csv_m['performance'],
        'computility': csv_m['computility'], 'ratio': csv_m['ratio'],
    }
    ...
    if bin_l2_entries:
        ...
        csv_l2 = {
            'bw': csv_m['l2_bw'], 'fops': csv_m['total_fops'],
    ...
    for label, bin_entry, csv_vals, bin_fops, bin_total_bytes in comparisons:
        ...
        key_map = {
            'Bandwidth (TB/s)': 'bw', 'fops': 'fops', 'total_bytes': 'total_bytes',
            'Arithmetic Intensity': 'AI', 'Performance (TOps/s)': 'performance',
            'Computility (TOps/s)': 'computility', 'Performance Ratio': 'ratio',
        }

        print(f"  {'Metric':<25} {'Bin (Method 1)':<22} {'CSV (Method 2)':<22} {'Match':<8} {'Rel.Diff':<12}")
    ...
    print("-" * 90)
    for e in bin_data['roofline_entries']:
        print(f"  {e['title']:<22} / {e['bw_name']:<22}:  bw={e['bw']:.6f}  AI={e['AI']:.6f}  "
